...
from internvl.train.dataset import (ConcatDataset, TCSLoader,
                                    ...
                                    preprocess_internlm, read_frames_decord,
...
class ModelArguments:
    ...
    unfreeze_lm_head: bool = field(
        default=False, metadata={
            'help': 'Set to True to unfreeze the head of LLM. Default is False.'}, )
...
class LazySupervisedDataset(Dataset):
    ...
    def get_preprocess_function(self):
        # Select the appropriate preprocessing function based on the template
        # name
    ...
    def multi_modal_get_item(self, data_item):
        ...
        # Apply the transformation to each image and stack the results into a
        # tensor
        pixel_values = [transform(image) for image in images]
        pixel_values = torch.stack(pixel_values)

        # Ensure that there is only one patch if dynamic image size is not
        # enabled
    ...
    def multi_modal_multi_image_get_item(self, data_item):
        ...
        num_image_tokens = [
            ...
            num_tile for num_tile in num_tiles]
        ret = preprocess_function(self.template_name,
                                  [deepcopy(data_item['conversations'])],
                                  self.tokenizer,
                                  num_image_tokens,
    ...
    def video_get_item(self, data_item):
        ...
        # frame_indices = np.linspace(0, len(vr)-1, num=self.max_num_frame, dtype=int)
        # image_list = [Image.fromarray(vr[i].asnumpy()) for i in frame_indices]
        # except ImportError:
        # raise ImportError("Failed to load video: decord or petrel_client is required for video loading.")
        # else:
    ...
    def pure_text_get_item(self, data_item):
        ...
        # Apply the transformation to each image patch and stack them into a
        # tensor
        pixel_values = [transform(image) for image in images]
        pixel_values = torch.stack(pixel_values)
        num_patches = pixel_values.size(0)
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        while True:
            ...
            try:
                ...
                if not isinstance(
                        e, (UnidentifiedImageError, FileNotFoundError)):
                    traceback.print_exc()
                data_item = json.loads(self.raw_data[i])
